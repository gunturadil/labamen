...
switch($_GET[act]){
  ...
  default:
	...
	echo "<div style='text-align:right;padding-right:30px;'>
		...
		echo"<datalist id='auto'>";
		 $qry=mysqli_query($conn, "SELECT * FROM tbl_mahasiswa");
    ...
    $tampil=mysqli_query($conn, "SELECT * FROM tbl_mahasiswa");
    $no=1;
  ...
  case "editmahasiswa":
  $sql=mysqli_query($conn, "SELECT * FROM tbl_mahasiswa WHERE id='$_GET[id]'");
 ...
 echo'<div algin=left><input type=button value=Back onclick=self.history.back()></div>';
    ...
    $tampil=mysqli_query($conn, "SELECT * FROM tbl_mahasiswa WHERE id LIKE '%$_POST[cari]%' or nama LIKE '%$_POST[cari]%'");
    $no=1;
    while ($r=mysqli_fetch_array($tampil)){
       echo "
	   		 <tr><td>$no</td>
